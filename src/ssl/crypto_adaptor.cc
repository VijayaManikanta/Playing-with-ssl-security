...
int aes_encrypt(const unsigned char* key, size_t key_len, const unsigned char* iv, size_t iv_len,
                std::string *cipher_text, const std::string &plain_text) {

  SecByteBlock aes_key(key, key_len);
  SecByteBlock aes_iv(iv, iv_len);

  try {
    CBC_Mode<AES>::Encryption aes_enc;
    aes_enc.SetKeyWithIV(aes_key, aes_key.size(), aes_iv);
...
int aes_decrypt(const unsigned char* key, size_t key_len, const unsigned char* iv, size_t iv_len,
  ...
  try {
    ...
    aes_dec.SetKeyWithIV(aes_key, aes_key.size(), aes_iv);
