...
void OutputResultBytes(const char *name, double length, double timeTaken)
{
	...
	std::cout << "\n<TR><TH>" << name;
//	std::cout << "<TD>" << setprecision(3) << length / (1024*1024);
	std::cout << setiosflags(std::ios::fixed);
//	std::cout << "<TD>" << setprecision(3) << timeTaken;
	std::cout << "<TD>" << std::setprecision(0) << setiosflags(std::ios::fixed) << mbs;
	if (g_hertz)
		std::cout << "<TD>" << std::setprecision(1) << setiosflags(std::ios::fixed) << timeTaken * g_hertz / length;
	logtotal += log(mbs);
	logcount++;
}
...
void OutputResultKeying(double iterations, double timeTaken)
{
	...
	std::cout << "<TD>" << std::setprecision(3) << setiosflags(std::ios::fixed) << (1000*1000*timeTaken/iterations);
	if (g_hertz)
		std::cout << "<TD>" << std::setprecision(0) << setiosflags(std::ios::fixed) << timeTaken * g_hertz / iterations;
}
...
void OutputResultOperations(const char *name, const char *operation, bool pc, unsigned long iterations, double timeTaken)
{
	...
	std::cout << "<TD>" << std::setprecision(2) << setiosflags(std::ios::fixed) << (1000*timeTaken/iterations);
	if (g_hertz)
		std::cout << "<TD>" << std::setprecision(2) << setiosflags(std::ios::fixed) << timeTaken * g_hertz / iterations / 1000000;
...
void BenchmarkAll(double t, double hertz)
{
	...
	std::cout << "Throughput Geometric Average: " << setiosflags(std::ios::fixed) << exp(logtotal/(logcount ? logcount : 1)) << std::endl;
